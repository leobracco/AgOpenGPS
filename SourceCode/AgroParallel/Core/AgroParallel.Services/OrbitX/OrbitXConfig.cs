...
namespace AgroParallel.OrbitX
{
    public class OrbitXConfig
    {
        ...
        // Token "master" compartido — solo se usa como fallback si el server
        // responde 401 "Dispositivo no registrado". Permite el auto-registro
        // definido en OrbitX-Server/routes/devices.js (deviceAuth: si el doc
        // no existe pero el token es master, se crea el doc).  Después del
        // primer alta el operador DEBE regenerar el token desde el panel.
        ...
        [JsonPropertyName("sync_flowx")]
        public bool SyncFlowX { get; set; }

        [JsonPropertyName("sync_stormx")]
        public bool SyncStormX { get; set; }
        ...
        // que están detrás del PC con PilotX en LAN.
        ...
        public OrbitXConfig()
        {
            ...
            DeviceToken = "";
            MasterToken = "vx-device-token";  // matchea DEVICE_MASTER_TOKEN del server
            ...
            FirmwareCacheDir = "";  // vacío → <PilotX>/firmware-cache
            FirmwareHttpPort = 8088;
            FirmwareSyncIntervalMin = 10;
            CamarasStreamingEnabled = false;
            // Por defecto publicamos al subdominio dedicado de cámaras —
            // orbitx.agroparallel.com pasa por Cloudflare proxy (naranja) que
            // bloquea TCP no-HTTP en el 8554. cam.agroparallel.com está en
            // DNS-only (gris) y va directo a MediaMTX.
