...
//   - Estado por surco:
//       * SeccionCortada (placeholder, requiere bridge AOG) → no-data
            ...
            lock (_lock)
            {
                ...
                if (_imp?.MapeoSensores != null)
                {
                    foreach (var sc in _imp.MapeoSensores)
                    {
                        ...
                        {
                            double objMin = tl.Objetivo; // sem/min ya en objetivo del tren
                            double tol = (_imp.Setup?.ToleranciaDesvio ?? 20) / 100.0;
                            if (objMin <= 0) surco.Estado = "ok";
                            else if (surco.Spm < objMin * (1 - tol))
                            {
                                surco.Estado = "bad";
                                surco.Alerta = true;
                            }
                            else if (surco.Spm > objMin * (1 + tol)) surco.Estado = "warn";
                            else surco.Estado = "ok";
                        }
                        tl.Surcos.Add(surco);
                    }
                }
                ...
                foreach (var tl in trenes.Values)
                {
                    ...
                    {
                        if (s.Estado == "ok" || s.Estado == "warn" || s.Estado == "bad") activos++;
                        if (s.Estado == "bad") fallas++;
