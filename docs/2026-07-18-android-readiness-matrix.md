...
| **14** | **Proceso único (merge PilotX+CoreX)** | **~70%** | Media | ~10 archivos de bootstrapping | En Android no hay 2 .exe. Todo dentro de 1 app: Activity (UI) + Foreground Service (broker + bridge + guiado). Primer paso (2026-07-22): `SourceCode/PilotX.GuidanceEngine` (net9.0 puro, sin WinForms/GL) — `GuidanceEngineHost` implementa las ~19 interfaces I*Host que hoy implementa FormGPS, orquestando los mismos CPositionUpdater/CHeadingUpdater/CAutoSteerUpdater/CYouTurnUpdater/CSectionCalculator/CSettingsSender del bloque 9. Segundo paso: `CoreXEngineHost` + `Net9SerialPortService` — arranca MqttBrokerService/UdpBridgeService/NtripClientService/los 6 puertos serie en el mismo proceso (flag `--corex`). Tercer paso: `GuidanceEngineHost.ExecuteCommand(string)` — mismo vocabulario que `FormGPS.ExecuteGuidanceCommand`/`IGuidanceCalculator.ExecuteCommand` ("autosteer"), primero por TCP de prueba en :15556. Cuarto paso: ese comando también por MQTT real (`CoreXEngineHost.SubscribeCommands()`, tópico `agp/aog/guidance/command`, mismo patrón que `FirmwareOtaClient`/`NodoRegistryService`). **Quinto paso hecho 2026-07-22**: `GuidanceEngineHost.OpenField(string)`/`CloseField()` (nuevo `GuidanceEngineHost.Job.cs`) — mismo flujo de datos que `FormGPS.FileOpenField`/`JobNew` (streamers `AgOpenGPS.IO` ya portables: `FieldPlaneFiles`/`TrackFiles`/`BoundaryFiles`, sin `OpenFileDialog` ni las ~30 asignaciones de botones WinForms de `JobNew`) — comandos nuevos `"job_start_<lote>"`/`"job_close"` sobre el mismo `RegistrySettings.fieldsDirectory` que usa PilotX real (`RegistrySettings.Load()` agregado al arranque de `Program.cs`; no toca Registry de Windows en net9.0 puro, guard ya existente). Verificado en runtime con `--sim` contra el lote real `66666` (el mismo de sesiones anteriores, en MyDocuments\AgOpenGPS\Fields): `job_start_66666` → `IsJobStarted=True` con 1 track cargado (la AB guardada), `job_close` → `IsJobStarted=False`, lote inexistente → `unknown` sin tocar estado. Compila limpio (0 warnings), build.ps1 completo OK, 141 tests verdes. Falta: consumidor real del otro lado (Android UI o Hub) que dispare estos comandos, ampliar vocabulario (youturn, section master) cuando exista ese consumidor, y probar NTRIP/serial con credenciales/hardware real. |
