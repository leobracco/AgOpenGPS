...
| **6** | **Render del mapa (OpenGL → GL ES)** | **~72%** | **Muy alta** | SourceCode/PilotX.Desktop/Views/MapGlSurface.cs | **NO se porta el GL inmediato de OpenGL.Designer.cs** — se reemplaza por el render Avalonia de **PilotX.Desktop** (`OpenGlControlBase` + Silk.NET, strangler-fig por polling REST a FormGPS). Stages 1→5 HECHOS: grid, coverage triangulado, guías AB/curva/contorno, secciones/implemento, tramlines, **y youturn + recorded paths (Stage 5, 2026-07-21: endpoint /api/aog/paths + FormGpsPathsCalculator + render 2 GL_LINE_STRIP)**. FALTA: **Stage 6** (cámara: zoom rueda/pan/rotate), **Stage 7** (retirar MapSkiaSurface legacy + toggle `--gl`). Carril taller. Pendiente validación visual con `--gl=on`. |
...
| **14** | **Proceso único (merge PilotX+CoreX)** | **~20%** | Media | ~10 archivos de bootstrapping | En Android no hay 2 .exe. Todo dentro de 1 app: Activity (UI) + Foreground Service (broker + bridge + guiado). **Primer paso hecho 2026-07-22**: `SourceCode/PilotX.GuidanceEngine` (net9.0 puro, sin WinForms/GL) — `GuidanceEngineHost` implementa las ~19 interfaces I*Host que hoy implementa FormGPS, orquestando los mismos CPositionUpdater/CHeadingUpdater/CAutoSteerUpdater/CYouTurnUpdater/CSectionCalculator/CSettingsSender del bloque 9. Verificado en runtime: el loop heading→posición→autosteer→youturn corre standalone (con CSim.DoSimTick interno Y contra un CoreX real por UDP loopback :15555/:17777), sin FormGPS. Falta: wire de MqttBrokerService/UdpBridgeService/NtripClientService de CoreX en el mismo proceso, y un origen de comando real para btnStates/autosteer (hoy sin UI detrás). Depende de bloques 8 y 9 (ambos ya avanzados). |
