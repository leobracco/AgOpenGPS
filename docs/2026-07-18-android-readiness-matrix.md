...
| **4** | **Lógica de guiado pura (clases C#)** | **80%** | Media | 5 archivos en AgOpenGPS.Core/Classes/ | Auditado 2026-07-19: el desacople de FormGPS **ya está hecho** — `mf` está tipado como interfaces `I*Host` (17 interfaces en Core/Interfaces/) en todas las clases de guiado; FormGPS las implementa vía partials adapter. Lo que queda es **separar el Draw GL embebido** de 5 clases (CABCurve 61 llamadas GL, CVehicle 60, CTool 42, CFence 36, CContour 28) a Visuals/ — trabajo que se solapa con el bloque 6. Ya sin OpenTK: CGLM, CTrack, CYouTurn, CRecordedPath y CTram (Draw movidos a DrawLib/GuidanceDrawExtensions.cs). |
