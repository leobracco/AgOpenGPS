...
| **8** | **CoreX extracción a servicios** | **60%** | Alta | ~30 archivos en SourceCode/AgIO/ | Broker MQTT: extraído Y wired (MqttBrokerService). Bridge UDP: extraído Y wired (UdpBridgeService — UDP.designer.cs es wrapper delgado, sockets loopback :17777↔:15555 + LAN :9999↔:8888 en netstandard). NTRIP: extraído Y wired (NtripClientService — NTRIPComm.Designer.cs es wrapper; el form conserva solo UI/metering + radio/serial-pass). Serial: interfaz ISerialPortService + impl Windows; falta rutear SerialComm por la interfaz. Serial → USB-OTG (`UsbSerialForAndroid`) en Android. |
