...
| **7** | **Shell Android (WebView host)** | **60%** | Baja | SourceCode/PilotX.Android/ | **Compila y empaqueta APK (2026-07-19)**: MainActivity + WebView a 127.0.0.1:5180, HubForegroundService con broker MQTTnet embebido + AgpWebHost (servicios netstandard reales; stubs Fase 1 para lo FormGPS-backed), wwwroot como assets (560 entradas, APK 32 MB), DataRootOverride a getExternalFilesDir. Falta: probar en tablet real, puente JS nativo (close-hub), y el `ConfigRoot` inyectable en Services (escriben en BaseDirectory, read-only en Android — ver README del proyecto). |
...
| **9** | **FormGPS → desacoplamiento** | **20%** | **Muy alta** | ~124 archivos en GPS/Forms/ | FormGPS.cs es el monolito central (~4000 líneas) + 46 forms + 17 partials I*Host ya hechos. Auditoría 2026-07-19: **Position.designer.cs (1616 ln, el GPS loop) tiene 0 llamadas GL y ~10 toques UI** (lblSpeed, TimedMessageBox, btnAutoSteer.PerformClick) — extraíble con el patrón I*Host; Sections.Designer.cs (962 ln) y UDPComm.Designer.cs (1230 ln) tampoco tocan GL. Hecho: definiciones PGN (11 clases CPGN_*) en Core/Protocols/PgnDefinitions.cs y **parser de PGNs entrantes extraído** (Protocols/PgnReceiver.cs + IPgnReceiveHost — los 9 PGNs de CoreX parsean en Core; UDPComm.Designer quedó en sockets+UI). Falta: GPS loop (Position.designer) y secciones a servicios. |
