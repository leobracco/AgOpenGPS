...
| **14** | **Proceso único (merge PilotX+CoreX)** | **~92%** | Media | ~10 archivos de bootstrapping | En Android no hay 2 .exe. Todo dentro de 1 app: Activity (UI) + Foreground Service (broker + bridge + guiado). Primer paso: `SourceCode/PilotX.GuidanceEngine` (net9.0 puro, sin WinForms/GL) — `GuidanceEngineHost` implementa las ~19 interfaces I*Host que hoy implementa FormGPS, orquestando CPositionUpdater/CHeadingUpdater/CAutoSteerUpdater/CYouTurnUpdater/CSectionCalculator/CSettingsSender del bloque 9. Segundo paso: `CoreXEngineHost` + `Net9SerialPortService` (flag `--corex`). Tercer paso: `GuidanceEngineHost.ExecuteCommand(string)` — mismo vocabulario que `FormGPS.ExecuteGuidanceCommand`/`IGuidanceCalculator.ExecuteCommand`, primero por TCP de prueba en :15556. Cuarto paso: ese comando también por MQTT real (tópico `agp/aog/guidance/command`). Quinto paso: `GuidanceEngineHost.OpenField(string)`/`CloseField()` — abrir/cerrar lote real (`"job_start_<lote>"`/`"job_close"`) sobre `RegistrySettings.fieldsDirectory`. **Sexto paso hecho 2026-07-22**: 2 comandos más del mismo vocabulario real de FormGPS — `"uturn"` (`ToggleYouTurn()`, mismo comportamiento que `btnAutoYouTurn_Click` sin el ícono) y `"pick"` (`SelectTrack()`, mismo comportamiento que `btnTrack_Click` para elegir guía — mismo nombre que `IGuidanceCalculator.ExecuteCommand("pick")`). Necesario porque sin "pick" `Trk.idx` se queda en -1 después de abrir un lote y ninguna guía queda activa (mismo comportamiento que FormGPS real: cargar el archivo no selecciona nada solo). Verificado en runtime con `--sim` contra el lote real `Lote 1` (boundary + 1 track reales, distinto del `66666` usado antes): secuencia completa `uturn` (sin boundary, se ignora) → `job_start_Lote 1` (`IsJobStarted=True`) → `uturn` (sin guía seleccionada, se ignora) → `pick` (`track idx=0/1`) → `uturn` (`ON`) → `uturn` (`OFF`) — las 4 ramas de guarda/toggle de `btnAutoYouTurn_Click` confirmadas una por una. Compila limpio (0 warnings), build.ps1 completo OK, 141 tests verdes. **Séptimo paso hecho 2026-07-22 — foco 3, integración real a PilotX.Android**: hasta acá el "consumidor real" que faltaba era el bloqueador de todo el vocabulario de comandos. Se resolvió parcialmente integrando `GuidanceEngineHost` **dentro de la app Android** (no solo el .exe de consola de Windows). Requirió separar el proyecto: `System.IO.Ports` (que necesita `CoreXEngineHost`/`Net9SerialPortService` para los 6 puertos serie) no restaura para `net9.0-android` (`NETSDK1047`, no hay target `net9.0/android-arm64`) — se creó **`PilotX.GuidanceEngine.Core`** (nuevo proyecto, solo `GuidanceEngineHost*.cs`, sin System.IO.Ports ni AgroParallel.Services) del que `PilotX.Android` toma un `ProjectReference` directo; `PilotX.GuidanceEngine` (el .exe de consola, con `CoreXEngineHost`/serial) ahora referencia a `PilotX.GuidanceEngine.Core` en vez de duplicar los archivos. En `HubBootstrap.cs`: se instancia un `GuidanceEngineHost` (mismo `dataDir` de la app) y se reemplazan 2 stubs por `GuidanceEngineLotesService`/`GuidanceEngineGuidanceCalculator` (`GuidanceEngineServices.cs`, mismo patrón que `FormGpsLotesService`/`FormGpsGuidanceCalculator` del lado Windows). Sin fix GPS real todavía (el loopback UDP de `GuidanceEngineHost.Start()` queda escuchando sin nada que le mande PGN — eso necesita CoreX/serial por USB-OTG, bloque 8, hardware pendiente), pero abrir/cerrar lote real y ejecutar comandos de guiado (autosteer/uturn/pick) desde el Hub Android ya funcionan de verdad, no contra un stub. Verificado: `dotnet build PilotX.Android.csproj` 0 errores (mismo warning preexistente de antes) + `AgOpenGPS.sln` completo 0 errores + `build.ps1` OK + 141 tests verdes. **Octavo paso hecho 2026-07-22 — resto de los stubs "de datos de guiado"**: nuevo `GuidanceEngineStateServices.cs` con 5 implementaciones más, mismo patrón (copiar la lógica de los adaptadores `FormGps*.cs` de Windows contra `GuidanceEngineHost` en vez de `FormGPS`) — `StubAogStateProvider`→`GuidanceEngineStateProvider` (`GetSnapshot()`: posición/velocidad/área trabajada/estados de autosteer-secciones-track-youturn-contour/boundary-headland/hidráulico/tram, más `GetEventLog()`; los demás métodos de esa interfaz — volcado "todos los ajustes", 4 gráficos en vivo, shift-pos, sim-coords, colores, shapefile — quedan con el mismo default vacío que el stub, documentado como pendiente: necesitan buffers rodantes o una capa de shapefile que el motor no tiene), `StubSectionControlService`→`GuidanceEngineSectionControlService`, `StubCoverageService`→`GuidanceEngineCoverageService` (lee `TriStripField`), `StubVehicleToolService`→`GuidanceEngineVehicleToolService` (vehículo/tool/IMU vía `Properties.Settings.Default`, igual que Windows — se sacó `readonly` de `GuidanceEngineHost.Vehicle`/`Tool` para poder recargarlos en caliente al guardar, mismo patrón que `_form.vehicle = new CVehicle(_form)`), `StubQuantiXRuntimeService`→`GuidanceEngineQuantiXRuntimeService` (reusa el nuevo `GuidanceEngineStateProvider` + `MotoresConfig.Load()`, sin depender de `GuidanceEngineHost` directo). De los ~9 stubs originales quedan solo 3, y a propósito: `StubShapefileService`, `StubPilotXUpdateService` (bloque 12, feature distinta), `StubSistemaService` (brillo/apagado — APIs Android, no datos de guiado). Verificado: `PilotX.Android.csproj` 0 errores (mismo warning preexistente), `AgOpenGPS.sln` completo 0 errores + 141 tests verdes, `build.ps1` OK. **Noveno paso hecho 2026-07-22 — verificado en runtime real, no solo compila**: había un emulador Android ya configurado en esta PC (`Medium_Phone_API_36.1`, AVD existente) — se aprovechó para pasar de "compila para Android" a "corre en Android". `dotnet build PilotX.Android.csproj -p:AndroidPackageFormat=apk`, `adb install`, arranque del emulador (~6.5 min cold boot) y lanzado el APK de verdad. **`HubBootstrap.Start()` corrió sin ninguna excepción** (logcat filtrado por PID: 0 crashes, 0 `System.*Exception`, solo un warning SELinux preexistente y no relacionado al guardar `aog_settings.json`) — se ve `"PilotX: Hub arriba en http://127.0.0.1:5180/"` en el log. Con `adb forward` + `curl` contra el Hub corriendo DENTRO del emulador: `GET /api/aog/state` devuelve el snapshot real completo de `GuidanceEngineStateProvider` (`fields_directory` apuntando correctamente a `/storage/emulated/0/Android/data/.../files/AgOpenGPS/Fields`, `num_sections`/`tool_width`/`section_positions` reales desde `Properties.Settings.Default`); `GET /api/lotes` devuelve `[]` (instalación limpia, sin crash); `GET /api/aog/guidance` devuelve el snapshot de `GuidanceEngineGuidanceCalculator`; **`POST /api/aog/guidance/command {"cmd":"autosteer"}` → `{"ok":true}`, y el snapshot siguiente confirma `is_auto_steer_on:true`** — el roundtrip completo HTTP→`ExecuteCommand`→estado interno→snapshot corriendo de verdad en Android, no en el .exe de consola de Windows. Esta es la validación más fuerte de todo el bloque 14 hasta ahora. Falta: probar en tablet física real (el emulador no tiene GPS/USB-OTG real), y NTRIP/serial con credenciales/hardware real. |
