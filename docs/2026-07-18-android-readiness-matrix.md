...
| **5** | **AgOpenGPS.Core → netstandard** | **90%** | Media | split csproj | Modelos y streamers ya sin `System.Drawing.Common`: BingMap guarda PNG crudo (`byte[]`), BingMapStreamer es IO plano de bytes; la decodificación PNG→Bitmap vive en Texture2D (capa GL). 2026-07-19: WorldGrid y Camera separados (modelo portable + visual GL-side); las interfaces I*Host ya no exponen tipos DrawLib (Font/Texture2D viven en ITextFontHost/IToolTexturesHost/IVehicleTexturesHost en DrawLib/IDrawAssetHosts.cs, FormGPS las implementa aparte). OpenTK y Bitmap quedan confinados a DrawLib/+Visuals/+Drawing/ — todo GL-side, se va con el bloque 6. Último paso: multi-target o split del csproj excluyendo la capa de render para netstandard2.0. |
