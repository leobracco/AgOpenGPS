...
| **12** | **Self-update** | **~75%** | Media | ~3 archivos nuevos | **Hecho 2026-07-23**: reemplaza `StubPilotXUpdateService` por `AndroidPilotXUpdateService.cs` (PilotX.Android). Reusa `PilotXSelfUpdate` (`AgroParallel.Services/OrbitX/`, el mismo motor que ya usa Windows) para catálogo OTA + descarga + verificación SHA256 — se parametrizó (`product`/`stagingRoot`/`payloadFileName`, con defaults que preservan el comportamiento Windows exacto) para no duplicar esa lógica; se agregó `SetCurrentVersion()`/`MarkApplying()` como puntos de extensión para otras plataformas. Android usa el producto de catálogo **`PilotXAndroid`** (distinto de `PilotX`/Windows) y descarga a `Context.FilesDir/Updates/<version>/payload.apk`. `Apply()` es 100% distinto a Windows (que spawnea un Updater.exe externo): dispara el instalador del sistema vía `Intent.ACTION_VIEW` + `FileProvider` (`REQUEST_INSTALL_PACKAGES`, sin MDM no hay silent-install — el usuario confirma "Instalar" en un diálogo nativo). Agregado `AndroidManifest.xml` (permiso + `<provider>` FileProvider) y `Resources/xml/file_paths.xml`. Verificado en runtime en el emulador: `HubBootstrap.Start()` sin excepciones con el manifest nuevo, `GET /api/pilotx/update/status` devuelve `current_version` real (`"1.0.23"`, leído de `PackageManager`, no un stub), `POST /api/pilotx/update/check` devuelve correctamente el error esperado `"Tractor no vinculado a OrbitX"` (mismo guard que Windows) sin crashear. Build completo (`AgOpenGPS.sln` + `PilotX.Android.csproj` + `build.ps1`) 0 errores, 141 tests verdes. **Falta para el 100%**: (1) que alguien con acceso al panel de OrbitX dé de alta el producto `PilotXAndroid` y suba un APK de prueba, para poder verificar el flujo completo Check→Download→Apply contra un catálogo real (hoy solo se verificó el camino "no vinculado"); (2) probar el diálogo de instalación en un dispositivo/emulador real (Download + Apply con un APK real en staging); (3) decidir si vale la pena ajustar `actualizar.js` (Hub HTML, carril taller) para el flujo Android — hoy asume que el servidor se reinicia solo tras Apply (como Windows), en Android el servidor sigue vivo mientras el usuario confirma el diálogo del instalador, así que el polling de "esperando que vuelva" no aplica igual (no es un bug, solo UX subóptima). |
