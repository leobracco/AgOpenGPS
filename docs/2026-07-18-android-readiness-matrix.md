...
| **6** | **Render del mapa (OpenGL → GL ES)** | **~65%** | **Muy alta** | SourceCode/PilotX.Desktop/Views/MapGlSurface.cs | **NO se porta el GL inmediato de OpenGL.Designer.cs** — se reemplaza por el render Avalonia de **PilotX.Desktop** (`OpenGlControlBase` + Silk.NET, strangler-fig por polling REST a FormGPS). Stages 1→4b HECHOS: grid, coverage triangulado, guías AB/curva/contorno, secciones/implemento, tramlines. FALTA: **Stage 5** (YouTurn paths + recorded paths), **Stage 6** (cámara: zoom rueda/pan/rotate), **Stage 7** (retirar MapSkiaSurface legacy + toggle `--gl`). Carril taller. (Aporte android: `DrawArrowTriangle` del lightbar a GLW.) |
