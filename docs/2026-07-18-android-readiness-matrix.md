...
| **14** | **Proceso único (merge PilotX+CoreX)** | **~50%** | Media | ~10 archivos de bootstrapping | En Android no hay 2 .exe. Todo dentro de 1 app: Activity (UI) + Foreground Service (broker + bridge + guiado). Primer paso (2026-07-22): `SourceCode/PilotX.GuidanceEngine` (net9.0 puro, sin WinForms/GL) — `GuidanceEngineHost` implementa las ~19 interfaces I*Host que hoy implementa FormGPS, orquestando los mismos CPositionUpdater/CHeadingUpdater/CAutoSteerUpdater/CYouTurnUpdater/CSectionCalculator/CSettingsSender del bloque 9. Segundo paso (2026-07-22): `CoreXEngineHost` + `Net9SerialPortService` — arranca MqttBrokerService/UdpBridgeService/NtripClientService/los 6 puertos serie en el mismo proceso que `GuidanceEngineHost` (flag `--corex`), mismo protocolo loopback (CoreX escucha :17777, contesta :15555). **Tercer paso hecho 2026-07-22**: `GuidanceEngineHost.ExecuteCommand(string)` — mismo vocabulario que `FormGPS.ExecuteGuidanceCommand`/`IGuidanceCalculator.ExecuteCommand` ("autosteer" hoy), servido por un mini servidor TCP línea-por-línea propio en :15556 (no HttpListener — no es portable a Linux/Android sin Kestrel). Es el origen de comando real que faltaba para btnStates/autosteer sin UI detrás; verificado en runtime con `--sim` (toggle repetido → "ok", comando desconocido → "unknown"). Compila limpio (0 warnings), 141 tests verdes. Falta: enchufar esto a un transporte real (MQTT del propio `CoreXEngineHost` o el Hub) en vez del TCP de prueba, y probar NTRIP/serial con credenciales/hardware real. |
