...
| **4** | **Lógica de guiado pura (clases C#)** | **85%** | Media | 3 archivos en AgOpenGPS.Core/Classes/ | Auditado 2026-07-19: el desacople de FormGPS **ya está hecho** — `mf` está tipado como interfaces `I*Host` (17 interfaces en Core/Interfaces/) en todas las clases de guiado; FormGPS las implementa vía partials adapter. Lo que queda es **separar el Draw GL embebido** de 3 clases (CABCurve 61 llamadas GL, CVehicle 60, CTool 42) a Visuals/ — trabajo que se solapa con el bloque 6. Ya sin OpenTK: CGLM, CTrack, CYouTurn, CRecordedPath, CTram, CContour y CFence/CBoundary (Draw movidos a DrawLib/GuidanceDrawExtensions.cs). |
| **5** | **AgOpenGPS.Core → netstandard** | **80%** | Media | 3 archivos .cs | Modelos y streamers ya sin `System.Drawing.Common`: BingMap guarda PNG crudo (`byte[]`), BingMapStreamer es IO plano de bytes; la decodificación PNG→Bitmap vive en Texture2D (capa GL). Quedan con Bitmap solo Texture2D/GeoTexture2D/WorldGrid — todos GL-side, se van con el bloque 6. `Color/Point/Size` restantes son System.Drawing.Primitives (portable). `Properties.Settings` de Core ya es la clase XML propia (portable), no la de Windows. |
