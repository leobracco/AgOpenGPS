...
| **12** | **Self-update** | **~92%** | Media | ~3 archivos nuevos | Reemplaza `StubPilotXUpdateService` por `AndroidPilotXUpdateService.cs` (PilotX.Android). Reusa `PilotXSelfUpdate` (`AgroParallel.Services/OrbitX/`, el mismo motor que ya usa Windows) para catálogo OTA + descarga + verificación SHA256 — se parametrizó (`product`/`stagingRoot`/`payloadFileName`, con defaults que preservan el comportamiento Windows exacto) para no duplicar esa lógica; se agregó `SetCurrentVersion()`/`MarkApplying()` como puntos de extensión para otras plataformas. Android usa el producto de catálogo **`PilotXAndroid`** (distinto de `PilotX`/Windows) y descarga a `Context.FilesDir/Updates/<version>/payload.apk`. `Apply()` es 100% distinto a Windows (que spawnea un Updater.exe externo): dispara el instalador del sistema vía `Intent.ACTION_VIEW` + `FileProvider` (`REQUEST_INSTALL_PACKAGES`, sin MDM no hay silent-install — el usuario confirma "Instalar" en un diálogo nativo). Agregado `AndroidManifest.xml` (permiso + `<provider>` FileProvider) y `Resources/xml/file_paths.xml`.
**Verificado end-to-end 2026-07-23 sin depender del panel real de OrbitX** (el usuario prefirió no bloquearse en eso — se armó un catálogo OTA mock local en vez de esperar acceso al panel): servidor Kestrel descartable sirviendo `/api/ota/catalogo`/`/api/ota/firmware/...` con el APK real recién compilado como "versión 9.9.9", `orbitX.json` de prueba escrito en el storage interno de la app (`adb shell run-as`, requiere build Debug) apuntando `server_url` a `http://10.0.2.2:8090` (alias NAT del emulador al host). Flujo completo confirmado real, no mockeado del lado cliente: **Check** → `phase=UpdateAvailable`, catálogo con hash/tamaño correctos; **Download** → 34 MB bajados por la red del emulador, SHA256 verificado, `staging_ready=true`; **Apply** → `Intent.ACTION_VIEW` con el `content://` del `FileProvider` lanzó de verdad el **`PackageInstaller` real de Android** (confirmado por `dumpsys activity activities`: `InstallStaging` → `PackageInstallerActivity`, con la URI/MIME type correctos en el Intent) — quedó esperando confirmación del usuario (no se completó el tap final por ANRs del emulador, ajeno al código: "System UI/Process system isn't responding", limitación de recursos de esta VM, no relacionado a PilotX). 0 excepciones de la app en logcat en todo el flujo. Build completo `AgOpenGPS.sln`+`PilotX.Android.csproj`+`build.ps1` 0 errores, 141 tests verdes. **Falta para el 100%**: completar el tap de "Instalar" en un dispositivo/emulador sin los problemas de ANR de este entorno (mecanismo ya demostrado, falta el último toque de UI), y decidir si vale ajustar `actualizar.js` (Hub HTML, carril taller) para el flujo Android — hoy asume que el servidor se reinicia solo tras Apply (como Windows); en Android el proceso sigue vivo mientras el usuario ve el diálogo del instalador, el polling de "esperando que vuelva" no aplica igual (no es un bug, es UX a pulir). La prueba contra el catálogo real de OrbitX (producto `PilotXAndroid` dado de alta en el panel) queda pendiente para cuando el usuario decida hacerlo — el cliente ya está completo y probado con un servidor real. |
