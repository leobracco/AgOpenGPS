...
| **14** | **Proceso único (merge PilotX+CoreX)** | **~85%** | Media | ~10 archivos de bootstrapping | En Android no hay 2 .exe. Todo dentro de 1 app: Activity (UI) + Foreground Service (broker + bridge + guiado). Primer paso (2026-07-22): `SourceCode/PilotX.GuidanceEngine` (net9.0 puro, sin WinForms/GL) — `GuidanceEngineHost` implementa las ~19 interfaces I*Host que hoy implementa FormGPS, orquestando los mismos CPositionUpdater/CHeadingUpdater/CAutoSteerUpdater/CYouTurnUpdater/CSectionCalculator/CSettingsSender del bloque 9. Segundo paso: `CoreXEngineHost` + `Net9SerialPortService` — arranca MqttBrokerService/UdpBridgeService/NtripClientService/los 6 puertos serie en el mismo proceso (flag `--corex`). Tercer paso: `GuidanceEngineHost.ExecuteCommand(string)` — mismo vocabulario que `FormGPS.ExecuteGuidanceCommand`/`IGuidanceCalculator.ExecuteCommand` ("autosteer"), primero por TCP de prueba en :15556. Cuarto paso: ese comando también por MQTT real (`CoreXEngineHost.SubscribeCommands()`, tópico `agp/aog/guidance/command`, mismo patrón que `FirmwareOtaClient`/`NodoRegistryService`). **Quinto paso hecho 2026-07-22**: `GuidanceEngineHost.OpenField(string)`/`CloseField()` (nuevo `GuidanceEngineHost.Job.cs`) — mismo flujo de datos que `FormGPS.FileOpenField`/`JobNew` (streamers `AgOpenGPS.IO` ya portables: `FieldPlaneFiles`/`TrackFiles`/`BoundaryFiles`, sin `OpenFileDialog` ni las ~30 asignaciones de botones WinForms de `JobNew`) — comandos nuevos `"job_start_<lote>"`/`"job_close"` sobre el mismo `RegistrySettings.fieldsDirectory` que usa PilotX real (`RegistrySettings.Load()` agregado al arranque de `Program.cs`; no toca Registry de Windows en net9.0 puro, guard ya existente). Verificado en runtime con `--sim` contra el lote real `66666` (el mismo de sesiones anteriores, en MyDocuments\AgOpenGPS\Fields): `job_start_66666` → `IsJobStarted=True` con 1 track cargado (la AB guardada), `job_close` → `IsJobStarted=False`, lote inexistente → `unknown` sin tocar estado. Compila limpio (0 warnings), build.ps1 completo OK, 141 tests verdes. **Sexto paso hecho 2026-07-22 (sesión taller)**: `EngineWebHost.cs` + 6 adapters net9 (`PilotX.GuidanceEngine/Adapters/`: EngineStateProvider/Coverage/ToolGeometry/Tram/Paths/Guidance) que implementan las interfaces que consume el `AgpWebHost` leyendo el modelo del `GuidanceEngineHost` (gemelos headless de los FormGps*Calculator, rename lowercase→PascalCase; el StateProvider stubbea los métodos satélite que eran ventanas WinForms). Flag `--webhost` levanta el `AgpWebHost` (netstandard2.0, reusado tal cual) sirviendo /api/aog/{state,coverage,tool,tram,paths,guidance} en :5180 — la MISMA API que sirve FormGPS. Con esto **PilotX.Desktop renderiza el mapa contra el engine headless, sin FormGPS**. En el camino se encontró y arregló el **bug que impedía GPS real**: `GuidanceEngineHost.Start()` no llamaba `PgnReceiverField.StartWatch()` (FormGPS sí, FormGPS.cs:849) → `udpWatch` parado → el gate `< UdpWatchLimit(70ms)` del `case 0xD6` descartaba TODOS los fixes antes de arrancar el watch → deadlock (el `--sim` andaba porque no pasa por ese gate). **Verificado end-to-end con ModSim real** (que simula el CoreX-ECU GPS/IMU/STEER) → CoreX → engine `--webhost`: `fix_quality` 0→8, posición/lat-lon reales llegando, PilotX.Desktop dibuja el triángulo en la posición real. Falta: consumidor de comandos del otro lado (Android UI o Hub) que dispare autosteer/job/youturn, embeber las barras del cockpit (PilotX.Cockpit.Bars) en PilotX.Desktop para crear guías desde el mapa nuevo, y probar NTRIP/serial con hardware real. |
