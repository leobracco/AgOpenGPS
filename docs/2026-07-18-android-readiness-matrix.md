...
| **7** | **Shell Android (WebView host)** | **65%** | Baja | SourceCode/PilotX.Android/ | **Compila y empaqueta APK (2026-07-19)**: MainActivity + WebView a 127.0.0.1:5180, HubForegroundService con broker MQTTnet embebido + AgpWebHost (servicios netstandard reales; stubs Fase 1 para lo FormGPS-backed), wwwroot como assets (560 entradas, APK 32 MB), DataRootOverride a getExternalFilesDir. Hecho también `AgpPaths.ConfigRoot` (61 usos de BaseDirectory en Services redirigidos; en Windows default idéntico, en Android FilesDir — guardar configs ya funciona). **2026-07-22 (sesión android, con autorización directa del usuario — cruza a carril taller)**: 2 de los ~9 stubs FormGPS-backed reemplazados por implementaciones reales (`GuidanceEngineServices.cs`) respaldadas por `GuidanceEngineHost` (bloque 14) — `StubLotesService`→`GuidanceEngineLotesService` (listar/abrir/cerrar lote real) y `StubGuidanceCalculator`→`GuidanceEngineGuidanceCalculator` (snapshot/geometría/comandos autosteer-uturn-pick). Ver detalle en fila bloque 14. Falta: probar en tablet real, puente JS nativo (close-hub), y los ~7 stubs restantes (vehículo/tool, cobertura, section control, shapefile, QuantiX runtime, update, sistema). |
