...
| **14** | **Proceso único (merge PilotX+CoreX)** | **~40%** | Media | ~10 archivos de bootstrapping | En Android no hay 2 .exe. Todo dentro de 1 app: Activity (UI) + Foreground Service (broker + bridge + guiado). Primer paso (2026-07-22): `SourceCode/PilotX.GuidanceEngine` (net9.0 puro, sin WinForms/GL) — `GuidanceEngineHost` implementa las ~19 interfaces I*Host que hoy implementa FormGPS, orquestando los mismos CPositionUpdater/CHeadingUpdater/CAutoSteerUpdater/CYouTurnUpdater/CSectionCalculator/CSettingsSender del bloque 9. **Segundo paso hecho 2026-07-22**: `CoreXEngineHost` (lado "CoreX" del proceso único) + `Net9SerialPortService` (reimplementación net9.0 de `ISerialPortService`, sin arrastrar el proyecto AgIO net48/WinForms) — arranca MqttBrokerService/UdpBridgeService/NtripClientService/los 6 puertos serie + `CNmeaParser`/`PgnFrameParser` (linkeados por archivo desde AgIO, ya portables) en el mismo proceso que `GuidanceEngineHost`, mismo protocolo loopback (CoreX escucha :17777, contesta :15555). Flag `--corex` en `Program.cs`. Verificado en runtime: `--sim --corex` arranca los 3 servicios sin excepciones y el heading sigue convergiendo igual (avgSpeed→4.4 km/h). Compila limpio (0 warnings), 141 tests verdes. Falta: un origen de comando real para btnStates/autosteer (hoy sin UI detrás) y probar NTRIP/serial con credenciales/hardware real. |
