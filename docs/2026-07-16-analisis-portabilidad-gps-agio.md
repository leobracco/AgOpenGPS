...
|---|---|---|---|---|
| 1 | Extraer core a librería sin UI | AVANZADO | ~40% | ~14 clases de Classes/ acopladas a `FormGPS` por constructor (CISOBUS…; CAHRS/CDubins/CFieldData/CModuleComm/CTram/CSim/CBoundary(+CFence/CTurn/CHead)/CSection/CPatches/CTool/CVehicle/CABLine/CABCurve/CContour/CGuidance/CYouTurn/CTrack/CRecordedPath/CNMEA ya movidas) → interfaces estilo `FormGps*Service`; y el parsing NMEA/PGN de AgIO (NMEA.Designer, PGN.Designer, UDP) |
...
| CABCurve.cs | 1811 | Generación de curvas AB y trazado de curvas suaves | ~~FormGPS~~ invertida con `IABCurveHost : IABLineHost` — **movida a Core 2026-07-17** (ABLine directo; PerformClick/TimedMessageBox/StanleyGuidanceCurve vía host) | PORTABLE |
| CABLine.cs | 568 | Lógica de líneas AB para guiado | ~~FormGPS~~ invertida con `IABLineHost` — **movida a Core 2026-07-17** (Tool/Tram/Vehicle/Bnd/Mc/Ahrs/Font directos; trk/yt/gyd granulares hasta que se muevan) | PORTABLE |
...
| CContour.cs | 671 | Líneas de contorno de campo | ~~FormGPS~~ → `IContourHost : IABLineHost` — **movido a Core 2026-07-17** | PORTABLE |
...
| CGuidance.cs | 412 | Guiado Stanley / Pure Pursuit / PID | ~~FormGPS~~ → `IGuidanceHost : IABLineHost` — **movido a Core 2026-07-17** | PORTABLE |
...
| CNMEA.cs | 55 | Parseo NMEA | ~~FormGPS~~ invertida con `INmeaHost` (AppModel/CSim/WorldGrid directos de Core) — **movida a Core 2026-07-17** | PORTABLE |
| CPatches.cs | 162 | Parches/áreas trabajadas (triángulos) | ~~FormGPS~~ invertida con `IPatchesHost` — **movida a Core 2026-07-17** (colores UI como vec3 vía host) | PORTABLE |
| CRecordedPath.cs | 849 | Grabación/replay de rutas Dubins | ~~FormGPS~~ invertida con `IRecordedPathHost : IYouTurnHost` (CYouTurn directo; sim/botonera detrás del host) — **movida a Core 2026-07-17** | PORTABLE |
...
| CTool.cs | 383 | Configuración de implemento (ancho, solape, ejes) | ~~FormGPS~~ invertida con `IToolHost` — **movida a Core 2026-07-17** (Sim/Mc/Section/Tram/VehicleConfig directos; texturas como `Texture2D`) | PORTABLE |
| CTrack.cs | 355 | Pistas de referencia AB/Curva | ~~FormGPS~~ → `ITrackHost : IGuidanceHost` — **movido a Core 2026-07-17** | PORTABLE |
...
| CVehicle.cs | 386 | Configuración de vehículo (ruedas, antena, PID) | ~~FormGPS~~ invertida con `IVehicleHost` — **movida a Core 2026-07-17** (Tool/Sim/Mc/Bnd directos; texturas como `Texture2D`) | PORTABLE |
| CYouTurn.cs | 2958 | Giros en U (generación y ejecución) | ~~FormGPS~~ → `IYouTurnHost : IGuidanceHost` — **movido a Core 2026-07-17** | PORTABLE |
